...
function glowingButton(props: GlowingButtonProps) {
  ...
  return (
    <button type={props.isSubmit ? "submit" : undefined} className={`flex py-1.5 ${props.displayArrow ? 'pl-3 pr-1' : 'px-2'} border border-venice-inner-bevel rounded-lg ${props.color} shadow-lg ${props.glowColor}`}>
      {icon}
